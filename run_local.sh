...
cd application/web
# cp -r can flatten node_modules/.bin symlinks (e.g. tsc -> ../typescript/bin/tsc)
if [[ -e node_modules/.bin/tsc && ! -L node_modules/.bin/tsc ]]; then
  echo "    Removing corrupted node_modules (broken .bin symlinks)"
  rm -rf node_modules
fi
