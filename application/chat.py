...
def _llm_gateway_settings() -> tuple[str, str] | None:
    """Load gateway URL/key from config.json. Returns None if incomplete."""
...
def uses_llm_gateway() -> bool:
    return _llm_gateway_settings() is not None
...
def _build_llm_gateway_chat(profile: dict, max_output_tokens: int):
    """Build chat model via gateway proxy (ChatAnthropic / ChatOpenAI)."""
    ...
    if not gw_model:
        logger.warning(
            "No gateway model mapping for %s; falling back to Bedrock",
            model_name,
        )
        return None
    ...
    logger.info(
        "Using gateway: url=%s model=%s type=%s",
        gateway_url,
        gw_model,
        model_kind,
    )
    ...
    logger.warning(
        "Gateway does not support model_type=%s for %s; falling back to Bedrock",
        model_kind,
        model_name,
    )
    return None
...
def _extract_text_from_pdf_bytes(data: bytes) -> str:
    try:
        ...
        logger.warning(f"pdfplumber extract failed, trying pypdf: {e}")
...
def _extract_text_from_document_bytes(data: bytes, file_type: str) -> str:
    """Best-effort text extraction for Load-files attachments."""
    if file_type in (
        "txt",
        "md",
        "markdown",
