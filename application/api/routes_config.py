...
def _gateway_from_config() -> tuple[str, str]:
...
def _probe_gateway_models(url: str, key: str, *, timeout: float = 15.0) -> dict:
    ...
    try:
        ...
        if response.status_code >= 400:
            logger.warning(
                "Gateway model probe failed: status=%s body=%s",
        ...
        logger.exception("Gateway model probe error")
        return {
            "ok": False,
...
def _gateway_ui_models() -> list[str]:
    ...
    url, key = _gateway_from_config()
    if not url or not key:
        return []

    result = _probe_gateway_models(url, key, timeout=5.0)
